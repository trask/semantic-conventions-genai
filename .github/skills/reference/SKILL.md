...
- Do not delay sampler-visible request attributes with `span.set_attribute` / `activity.SetTag` when their values are already available at span or activity creation time. This includes operation name, provider, request model and parameters, server address/port, input messages, system instructions, tool definitions, agent/workflow identity, and tool call arguments. Keep response values, generated IDs, token usage, output messages, retrieval results, and tool results after the operation that produces them.
