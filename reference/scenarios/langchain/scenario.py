...
def run_plan_with_repair_reference():
    """Scenario: planning phase that triggers OutputFixingParser repair.

    Demonstrates the multi-chat-call planning pattern: when the first LLM
    response is malformed, langchain's `OutputFixingParser` issues a second
    LLM call wrapping the malformed completion in `NAIVE_FIX_PROMPT` (see
    `langchain_classic/output_parsers/prompts.py`) and asks the model to
    repair its own output. The two calls land as TWO `chat` spans under
    ONE `plan` span, which is the structural reason planning is modeled as
    a span (parent boundary over N child LLM calls) rather than a boolean
    attribute on a single chat span.

    Default `PlanningOutputParser` does not raise on empty parses (it
    silently returns `Plan(steps=[])`), so we subclass it to raise
    `OutputParserException` when no steps are produced — that is what
    triggers `OutputFixingParser.parse` to invoke `retry_chain`. The mock
    server returns malformed text on the first request (sentinel
    `[FORCE_MALFORMED]`) and a valid plan on the repair request (detected
    via the literal NAIVE_FIX wrapper text).
    """
    print("  [plan] Plan-and-Execute with OutputFixingParser repair (reference implementation)")
    ...
    class StrictPlanningOutputParser(PlanningOutputParser):
        """Variant that raises on empty parses so OutputFixingParser engages."""

        def parse(self, text):
            plan = super().parse(text)
            if not plan.steps:
                msg = f"PlanningOutputParser produced no steps from: {text!r}"
                raise OutputParserException(msg)
            return plan

        def get_format_instructions(self) -> str:
            # Required by NAIVE_FIX_PROMPT — supplies the {instructions}
            # template variable so the repair prompt renders cleanly.
            return (
                "Output a plan as a numbered list of steps starting with the "
                "header 'Plan:' followed by lines like '1. ...' and ending "
                "with '<END_OF_PLAN>'."
            )

    planner = load_chat_planner(chat_model)
    planner.output_parser = OutputFixingParser.from_llm(
        llm=chat_model,
        parser=StrictPlanningOutputParser(),
        max_retries=1,
    )
    ...
    with _reference_tracer.start_as_current_span("plan") as plan_span:
        ...
        chat_model.root_client.chat.completions.create = _capture_create
        try:
            # The [FORCE_MALFORMED] sentinel routes the first plan call to
            # the mock's malformed branch, causing StrictPlanningOutputParser
            # to raise and OutputFixingParser to issue a second LLM call.
            plan = planner.plan(inputs={"input": "[FORCE_MALFORMED] What is the capital of France?"})
        finally:
            chat_model.root_client.chat.completions.create = original_create

        # Emit one chat span per captured call. With OutputFixingParser
        # engaged we expect exactly two: the original planner request and
        # the repair request.
        for messages, completion in zip(captured_messages_per_call, captured_completions, strict=True):
            with _reference_tracer.start_as_current_span("chat gpt-4o-mini") as chat_span:
                ...
                parsed = completion.parse() if hasattr(completion, "parse") else completion
                if getattr(parsed, "model", None):
                    chat_span.set_attribute("gen_ai.response.model", parsed.model)
                if getattr(parsed, "id", None):
                    chat_span.set_attribute("gen_ai.response.id", parsed.id)
                if getattr(parsed, "choices", None):
                    chat_span.set_attribute(
                        "gen_ai.response.finish_reasons",
                        [choice.finish_reason for choice in parsed.choices if choice.finish_reason],
                    )
                    assistant_content = parsed.choices[0].message.content
                    if assistant_content:
                        chat_span.set_attribute(
                            "gen_ai.output.messages",
                            json.dumps(
                ...
                if getattr(parsed, "usage", None):
                    chat_span.set_attribute("gen_ai.usage.input_tokens", parsed.usage.prompt_tokens)
                    chat_span.set_attribute("gen_ai.usage.output_tokens", parsed.usage.completion_tokens)

        print(f"    -> planned {len(plan.steps)} step(s) after {len(captured_completions)} chat call(s)")
...
def main():
    ...
    run_plan_with_repair_reference()
